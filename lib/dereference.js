...
const lib = {
   ...
   */
  recurse (recurseFunc, subSchema, params) {
  ...
  dereferenceSubSchema (params) {
    ...
    let path
    try {
      path = lib.getPath(subSchemaRef)
    } catch (e) {
      utils.addErrorResult(results, subSchemaRef, e.message)
      return
    }
    ...
    try {
      lib.processRef(subSchema, params)
    } catch (e) {
      utils.addErrorResult(results, `${subSchemaRef}/$ref`, e.message)
      return
    }

    lib.recurse(lib.dereferenceSubSchema, subSchema, params)
  },
  ...
  dereference (src) {
    ...
    return {
      schema: dest,
      errors: utils.aggregateResults(results).errors,
      refs: expandedRefs
    }
  }
}
