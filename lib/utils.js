...
const utils = {
   ...
   */
  doesModelContainRequiredField (model) {
    if (_.isArray(model)) {
      for (let i = 0, len = model.length; i < len; i++) {
        if (utils.doesModelContainRequiredField(model[i])) {
          return true
        }
      }
    ...
    for (let key in model) {
      if (utils.doesModelContainRequiredField(model[key])) {
        return true
      }
    }
  },
   ...
   */
  getModelPath (reference, dependencyReference) {
    const pattern = /^[^\.](.*[^\.])?$/
   ...
   */
  getSubModel (model, reference, dependencyReference) {
    const path = utils.getModelPath(reference, dependencyReference)
   ...
   */
  getInitialValue (id, formValue, initialValue, model, defaultValue) {
   ...
   */
  findValue (obj, valuePath, startPath) {
    startPath = startPath || ''
   ...
   */
  parseVariables (valueObj, queryJSON, startPath) {
    startPath = startPath || ''
    if (queryJSON.indexOf('${') !== -1) {
      const valueVariable = queryJSON.split('${')[1].split('}')[0]
      const result = utils.findValue(valueObj, valueVariable, startPath)
      ...
      return utils.parseVariables(valueObj, newQueryJson, startPath)
    }
    return queryJSON
  },
   ...
   */
  populateQuery (valueObj, query, startPath) {
    return JSON.parse(utils.parseVariables(valueObj, JSON.stringify(query), startPath))
  },
   ...
   */
  hasValidQueryValues (value, queryDef, startPath) {
    if (!queryDef) {
      return true
    }

    try {
      const query = utils.populateQuery(value, queryDef, startPath)
