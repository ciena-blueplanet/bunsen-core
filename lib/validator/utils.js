...
const utils = {
   ...
   */
  validateRequiredAttribute (object, path, attribute, possibleValues) {
    const errors = []
   ...
   */
  addErrorResult (results, path, message) {
    if (path === undefined) {
   ...
   */
  addWarningResult (results, path, message) {
    results.push({
      errors: [],
   ...
   */
  validateJsonString (jsonStr, jsonObj) {
    const result = {
      errors: [],
      warnings: []
    }
   ...
   */
  ensureJsonObject (json) {
    let strResult = null
    if (_.isString(json)) {
      const jsonStr = json
      try {
        json = JSON.parse(jsonStr)
        strResult = utils.validateJsonString(jsonStr, json)
      } catch (e) {
        return [
          undefined,
          {
